...
public class LC1744 {
    ...
    public boolean[] canEat(int[] candiesCount, int[][] queries) {
        ...
        for (int[] query : queries) {
            int beforeFavorite = sum(candiesCount, query[0]);
            int sumCandies = beforeFavorite + candiesCount[query[0]];
            answer[i++] = sumCandies >= (query[1] + 1) && (long) (query[1] + 1) * query[2] > beforeFavorite;
        }
